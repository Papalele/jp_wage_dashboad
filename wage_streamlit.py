...
st.header("2019年：一人当たりの平均賃金のヒートmap")
...
df_pref_mean = df_pref_mean.rename(columns={"一人当たり賃金（万円）":"都道府県_一人当たり賃金（万円）"})


pref_list = df_pref_mean["都道府県名"].unique()

option_pref = st.selectbox("下記から選んでください",options= pref_list)
...
df_mean_line = df_mean_line[["集計年","全国_一人当たり賃金（万円）","都道府県_一人当たり賃金（万円）"]]
df_mean_line = df_mean_line.set_index("集計年")
st.line_chart(df_mean_line)


#バブルチャートの作成
st.header("年齢階級別全国の平均賃金（万円）")
...
wage_mean_categ = df_jp_category[(df_jp_category["集計年"]== option_year)]
max_x = wage_mean_categ[option_wage].max() +50#グラフ化した時に見えやすいようにするため５０をプラス
